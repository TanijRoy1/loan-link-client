  ...
  const { data: activeLoans = {}, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosSecure.get(
        `/applications/stats/borrower?email=${user?.email}`
      );
      return res.data;
    },
  });
  ...
  return (
    <div className="py-6 space-y-6">
      <header className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold">Your Dashboard, {user?.displayName}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        <div className="p-4 bg-primary/10 rounded-2xl shadow">
          ...
          <div className="text-2xl font-bold">৳ {totalAmount || 0}</div>
              ...
              {payments.map((payment, i) => (
                <tr key={payment._id}>
                  ...
                  <td className="text-left">
                    ...
                    <p className="text-sm text-accent-content truncate max-w-md">
                      {payment.applicationId}
                    </p>
                  </td>
                  <td className="text-left truncate max-w-xs">
